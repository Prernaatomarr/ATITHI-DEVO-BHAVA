...
<table id="datatable" class="table table-striped table-bordered" >
	...
	while($rs = mysqli_fetch_array($qsql))
	{
		...
		{
			echo "
			<A href='viewfeedback.php?delid=$rs[0]' class='btn btn-danger' onclick='return confirmdel()'>Delete</a>";	
			if($rs['tourism_placeid'] != 0)
			{
			echo "<A href='feedbackpanel.php?location_id=$rs[tourism_placeid]' class='btn btn-primary' target='_blank' >View</a>";
			}
			if($rs['hotel_id'] != 0)
			{
			echo "<A href='hoteldetails.php?hotelid=$rs[hotel_id]'  class='btn btn-primary' target='_blank'>View</a>";
			}
		}		
		echo "		
